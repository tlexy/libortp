...
const static char *help="usage: mrtpsend	filename ip port nstreams [--packet-size size] [--ts-inc value]\n";
	...
	while( ((nCounter= (int) fread(m_pBuffer,1,m_nPacket_Size,infile))>0) && (m_bExit == FALSE) )
	{
		...
		for (k=0;k<m_nChannels;k++)
		{
			...
			if (session_set_is_set(pSessionSet,m_Session[k]))
			{
				...
				rtp_session_send_with_ts(m_Session[k],(const uint8_t*)m_pBuffer,nCounter,m_nUser_Timestamp);
